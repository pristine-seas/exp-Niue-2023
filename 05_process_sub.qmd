...
sub_fieldbook <- readxl::read_excel(file.path(exp_path, 
                                        paste0("data/primary/raw/sub/", exp_id, "_sub_fieldbook.xlsx"))) |> 
  as_tibble() |> 
  janitor::clean_names() |> 
...
sub_meta <- sub_fieldbook |> 
  mutate(ps_expedition_id = "NIU_2023", 
         method = "Submersible dive",
         ps_site_id = str_replace(ps_site_id, "NIU", "NIU_2023"),
         ...
         dive_start_time = lubridate::ymd_hms(dive_start_time, tz = "Pacific/Niue"),
         dive_end_time = lubridate::ymd_hms(dive_end_time, tz = "Pacific/Niue"),
...
all_dives <- all_dives |> 
  mutate(deepsee_dive_id = as.integer(deepsee_dive_id)) |> 
  bind_rows(sub_meta) |> 
  arrange(dive_start_time)
